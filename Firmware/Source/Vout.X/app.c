...
void interrupt isr(void) {
    if(TMR0IE && TMR0IF) { //if enabled and triggered
        ...
        bitSet(latC, 3); //LED_A1
        bitSet(latA, 4); //LED_A2
        bitSet(latA, 3); //LED_A3
        bitSet(latB, 5); //LED_A4
        cathodeBit(latA, 5, Display[DisplayIndex] & 0x01); //LED_C1
        cathodeBit(latA, 7, Display[DisplayIndex] & 0x02); //LED_C2
        cathodeBit(latA, 6, Display[DisplayIndex] & 0x04); //LED_C3
        cathodeBit(latC, 4, Display[DisplayIndex] & 0x08); //LED_C4
        cathodeBit(latC, 5, Display[DisplayIndex] & 0x10); //LED_C5
        cathodeBit(latC, 2, Display[DisplayIndex] & 0x20); //LED_C6
        cathodeBit(latC, 1, Display[DisplayIndex] & 0x40); //LED_C7
        cathodeBit(latC, 0, Display[DisplayIndex] & 0x80); //LED_C8

        switch (DisplayIndex) { //turn off
            case 0: bitSet(TRISB, 5); break;
            case 1: bitSet(TRISC, 3); break;
            case 2: bitSet(TRISA, 4); break;
            case 3: bitSet(TRISA, 3); break;
        }
        ...
        switch (DisplayIndex) { //turn on
            case 0: bitClear(TRISC, 3); break;
            case 1: bitClear(TRISA, 4); break;
            case 2: bitClear(TRISA, 3); break;
